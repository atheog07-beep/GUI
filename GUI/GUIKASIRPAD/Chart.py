...
class Chart(tk.Toplevel): #inisiasi class chart yang punya window terpisah
    ...
    def build(self):
        ...
        #untuk tampilin chart-nya
        self.tampil_chart()  
        #button untuk tutup window chart
    ...
    def tampil_chart(self):
        #baca file laporan_penjualan.csv dalam dataFrame dan hanya ambil kolom tanggal, nama, jumlah, subtotal
        df = pd.read_csv(self.laporan.file, usecols=["tanggal", "nama", "jumlah", "subtotal"])
        #update tanggal, hari, dan bulannya
        df["tanggal"] = pd.to_datetime(df["tanggal"])
        df["hari"]    = df["tanggal"].dt.day_name()
        df["bulan"]   = df["tanggal"].dt.strftime("%b %Y")
        #menghitung total jumlah penjualan setiap produk 
        per_produk  = df.groupby("nama")["jumlah"].sum().sort_values(ascending=False)
        #menghitung total pendapatan pertanggal
        per_tanggal = df.groupby(df["tanggal"].dt.strftime("%d %b"))["subtotal"].sum()
        urutan_hari = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"] #list urutan hari 
        label_hari  = ["Sen","Sel","Rab","Kam","Jum","Sab","Min"] #list hari dalam bahasa indonesia
        #hitung jumlah pendapatan perhari
        per_hari    = df.groupby("hari")["subtotal"].sum().reindex(urutan_hari, fill_value=0)
        per_bulan   = df.groupby("bulan")["subtotal"].sum() #hitung jumlah pendapatan bulan
        #membuat area grafik 2x2
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(10, 7), facecolor="white")
        fig.subplots_adjust(hspace=0.5, wspace=0.4)
        #grafik pie produk paling laris
        ...
        #grafik bar untuk penjualan pertanggal
        ...
        #grafik bar untuk penjualan perhari
        ...
        #grafik bar untuk penjualan perbulan
        ...
        #berikan format rupiah pada label harga
        fmt = mticker.FuncFormatter(lambda x, _: f"Rp {int(x):,}".replace(",", "."))
        #pengaturan mticker matplotlib
